...
(defun create-title-page ()
  ...
  (with-current-buffer (get-buffer-create html-buffer-name)
    (create-html-header))
  (setq current-line (concat html-author-paragraph-prefix author html-paragraph-postfix "\n"))
  (html-insert-line current-line)
  (setq current-line (concat html-title-paragraph-prefix title))
  (unless (eq title-archaic "")
    (setq current-line (concat current-line " (" title-archaic ")")))
